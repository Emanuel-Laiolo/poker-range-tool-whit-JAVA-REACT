...
export function normalizeHandActions(actions) {
  // Ensure valid shape + weights sum to 100.
  ...
  // Fix floating error by adjusting last.
...
export function validateRange(range) {
  ...
  for (const [hand, actions] of Object.entries(range.hands)) {
    ...
    if (Math.abs(sum - 100) > 0.001) throw new Error(`Hand ${hand} weights do not sum to 100`);
  }
